...
When operated mode is available, the extension reads **detection and enrichment metadata** from the operator status ConfigMap (`status.argocd`: `detected`, namespace, version, `lastChecked`, `resourceTreeCapable`, optional `resourceTreeLastError`). Per-application **resource-tree** payloads are fetched on-demand via `kubectl exec` → `kube9-operator query argocd resource-tree get <appName> --namespace=<appNamespace>` when the Application graph opens or refreshes **and** `resourceTreeCapable` is true. When capability is false or omitted, the extension hard-skips the operator tier and sets `limitedTopologyReason: operator_not_capable` if the graph falls back to limited topology. Application CRD reads remain the sync/health baseline.

The operator peer calls Argo CD **`GET /api/v1/applications`** for status cycles and **`GET /api/v1/applications/{name}/resource-tree`** for topology enrichment. It returns **raw Argo CD resource-tree JSON** to kube9-vscode; the extension assembles `ApplicationResourceGraph` via `OperatorResourceTreeClient` + `buildResourceTreeApplicationResourceGraph` and emits `topologySource: argocd_resource_tree` on success. Full tree payloads do **not** belong in the status ConfigMap (1 MiB limit).
