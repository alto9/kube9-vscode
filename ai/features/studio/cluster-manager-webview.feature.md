...
Scenario: Cluster Manager shows current active cluster
...
Scenario: Cluster Manager displays existing customizations
...
Scenario: Cluster Manager supports theme switching
  Given the Cluster Manager is open
  And VS Code is using light theme
  When the user switches VS Code to dark theme
  Then the Cluster Manager updates to dark theme
  And all colors and icons adapt to dark theme
  And text remains readable

Scenario: Only one Cluster Manager can be open
  Given the Cluster Manager is already open
  When the user executes "Kube9: Cluster Manager" again
  Then the existing Cluster Manager panel is revealed and focused
  And no new panel is created
  And the panel shows the current state of customizations

Scenario: Cluster Manager retains state when hidden
  Given the Cluster Manager is open
  And the user has unsaved search text "production"
  When they switch to a different editor tab
  And the Cluster Manager panel is hidden
  And they return to the Cluster Manager panel
  Then the search text "production" is still present
  And filtered results are still displayed

Scenario: Cluster Manager updates when kubeconfig changes
  Given the Cluster Manager is open showing 3 clusters
  When a new cluster is added to kubeconfig externally
  Then the Cluster Manager automatically updates
  And the new cluster appears in the list
  And the new cluster has default customizations (no folder, no alias, visible)

Scenario: Cluster Manager toolbar displays action buttons
  Given the Cluster Manager is open
...
Scenario: Search filters clusters by name
  Given the Cluster Manager displays 10 clusters
...
Scenario: Search filters clusters by original context name
  Given the Cluster Manager displays a cluster with alias "Prod"
...
Scenario: Cluster Manager shows status footer
  Given the Cluster Manager is open with 10 total clusters
...
Scenario: Cluster Manager handles empty kubeconfig
...
Scenario: Cluster Manager handles corrupted customization configuration
  ...
  And the Cluster Manager loads with default empty customizations
  And all clusters appear at root level without aliases

Scenario: Keyboard navigation in Cluster Manager
  Given the Cluster Manager is open with multiple clusters
...
Scenario: Cluster Manager displays inactive clusters
